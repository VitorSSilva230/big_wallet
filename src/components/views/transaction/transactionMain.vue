...
<template>
    <v-app>
        <v-container>
            <v-row>
                <v-col>
                 <span class="text-h5">Principais moedas no mercado</span>
                </v-col>
            </v-row>
            <v-row>
                <v-col>
                    <v-autocomplete v-model="selectedCoin.first" :items="firstCoinList" label="Escolha uma Moeda"
                        item-text="name" item-value="id" @change="getInfoCoin(selectedCoin.first)" solo-inverted>
                        <template #item="{ item }">
                            <v-list-item-avatar>
                                <v-img :src="item.image" alt="Coin Image"></v-img>
                            </v-list-item-avatar>
                            <v-list-item-content>
                ...
                <v-col>
                    <v-autocomplete v-model="selectedCoin.second" :items="secondCoinList" label="Escolha uma Moeda"
                        item-text="name" item-value="id" @change="getInfoCoin(selectedCoin.second)" solo-inverted
                        :disabled="!selectedCoin.first">
                        <template #item="{ item }">
                            <v-list-item-avatar>
                                <v-img :src="item.image" alt="Coin Image"></v-img>
                            </v-list-item-avatar>
            ...
            <p> Deseja acessar a lista completa de criptomoedas ? </p>
            <v-btn @click="retornaListaCompleta()">
...
export default {
    ...
    methods: {
        ...
        getIdFirstCoin() {
            ...
            axios.get(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=brl&order=market_cap_desc&per_page=300&page=1`)
                .then((response) => {
